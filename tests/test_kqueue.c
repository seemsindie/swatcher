...
#define TEST_DIR "test_kqueue_tmp"
...
/* ========== Helper: create watcher with kqueue (default) backend ========== */
...
static void test_kqueue_backend_exists(void)
{
    const swatcher_backend *b = swatcher_backend_kqueue();
    assert(b != NULL);
    assert(strcmp(b->name, "kqueue") == 0);
}

static void test_kqueue_detect_create(void)
{
    create_test_dir();
...
static void test_kqueue_detect_modify(void)
{
    create_test_dir();
    ...
    /* Watch the file directly for MODIFIED — kqueue gives per-fd events */
    swatcher_target_desc desc = {
        .path = filepath,
        .is_recursive = false,
        .events = SWATCHER_EVENT_ALL,
...
static void test_kqueue_detect_delete(void)
{
    create_test_dir();
    ...
    /* Watch the file directly for delete events */
    swatcher_target_desc desc = {
        .path = filepath,
        .is_recursive = false,
        .events = SWATCHER_EVENT_ALL,
...
static void test_kqueue_detect_move(void)
{
    create_test_dir();
    ...
    /* Watch the file directly for rename events */
    swatcher_target_desc desc = {
        .path = filepath_old,
        .is_recursive = false,
        .events = SWATCHER_EVENT_ALL,
    ...
    assert(get_event_count() > 0);
    assert(has_event(SWATCHER_EVENT_MOVED, "before.txt"));
...
static void test_kqueue_recursive_dynamic_mkdir(void)
{
    create_test_dir();
...
static void test_kqueue_recursive_dynamic_rmdir(void)
{
    create_test_dir();
...
static void test_kqueue_pattern_filtering(void)
{
    create_test_dir();
...
static void test_kqueue_coalesce(void)
{
    create_test_dir();
    ...
    /* Watch the file directly for MODIFIED events */
    swatcher_target_desc desc = {
        .path = filepath,
        .is_recursive = false,
...
static void test_kqueue_fd_limit_warning(void)
{
    /* This test verifies the backend initializes and reports fd limits correctly.
     * We don't actually exhaust fds — just verify the backend handles the limit field. */
    const swatcher_backend *b = swatcher_backend_kqueue();
    assert(b != NULL);
    ...
    /* Verify the backend stored a positive max_fds from getrlimit */
    swatcher_internal *si = SW_INTERNAL(sw);
    assert(si->backend_data != NULL);

    /* We can't directly access swatcher_kqueue from here without including the .c,
     * but we can verify init succeeded and the backend is functional */
    assert(si->backend != NULL);
    assert(strcmp(si->backend->name, "kqueue") == 0);
...
int main(void)
{
    ...
    printf("=== kqueue Backend Tests ===\n");

    RUN_TEST(test_kqueue_backend_exists);
    RUN_TEST(test_kqueue_detect_create);
    RUN_TEST(test_kqueue_detect_modify);
    RUN_TEST(test_kqueue_detect_delete);
    RUN_TEST(test_kqueue_detect_move);
    RUN_TEST(test_kqueue_recursive_dynamic_mkdir);
    RUN_TEST(test_kqueue_recursive_dynamic_rmdir);
    RUN_TEST(test_kqueue_pattern_filtering);
    RUN_TEST(test_kqueue_coalesce);
    RUN_TEST(test_kqueue_fd_limit_warning);
