...
#define TEST_DIR "test_fsevents_tmp"
...
/* ========== Helper: create watcher with FSEvents backend ========== */

static swatcher *create_fsevents_watcher(swatcher_config *config)
{
    swatcher *sw = malloc(sizeof(swatcher));
    assert(sw != NULL);
    assert(swatcher_init(sw, config));

    /* Swap from default (kqueue) to fsevents */
    swatcher_internal *si = SW_INTERNAL(sw);
    si->backend->destroy(sw);
    si->backend = swatcher_backend_fsevents();
    assert(si->backend->init(sw));
...
static void test_fsevents_backend_exists(void)
{
    const swatcher_backend *b = swatcher_backend_fsevents();
    assert(b != NULL);
    assert(strcmp(b->name, "fsevents") == 0);
}

static void test_fsevents_detect_create(void)
{
    create_test_dir();
    ...
    swatcher *sw = create_fsevents_watcher(&config);
...
static void test_fsevents_detect_modify(void)
{
    create_test_dir();
...
static void test_fsevents_detect_delete(void)
{
    create_test_dir();
...
static void test_fsevents_detect_move(void)
{
    create_test_dir();
    ...
    assert(get_event_count() > 0);
    /* FSEvents reports rename for both old and new names */
...
static void test_fsevents_recursive(void)
{
    create_test_dir();
    ...
    snprintf(subdir, sizeof(subdir), "%s/subdir", test_dir_abs);
    mkdir(subdir, 0755);
    sw_sleep_ms(400);

    assert(has_event(SWATCHER_EVENT_CREATED, "subdir"));

    reset_events();
    char filepath[SW_PATH_MAX];
    snprintf(filepath, sizeof(filepath), "%s/subdir/nested.txt", test_dir_abs);
    write_file(filepath, "nested content");
    sw_sleep_ms(600);

    /* FSEvents is inherently recursive — should see nested file events */
    assert(get_event_count() > 0);
...
static void test_fsevents_recursive_rmdir(void)
{
    create_test_dir();
...
static void test_fsevents_pattern_filtering(void)
{
    create_test_dir();
...
static void test_fsevents_coalesce(void)
{
    create_test_dir();
    ...
    /* Watch the directory for MODIFIED events */
    swatcher_target_desc desc = {
        .path = test_dir_abs,
        .is_recursive = false,
...
static void test_fsevents_file_target(void)
{
    create_test_dir();
    ...
    char filepath[SW_PATH_MAX];
    snprintf(filepath, sizeof(filepath), "%s/watched.txt", test_dir_abs);
    write_file(filepath, "initial");

    /* Also create a file we DON'T watch */
    char other_path[SW_PATH_MAX];
    snprintf(other_path, sizeof(other_path), "%s/unwatched.txt", test_dir_abs);
    write_file(other_path, "other");

    swatcher_config config = { .poll_interval_ms = 100, .enable_logging = false };
    swatcher *sw = create_fsevents_watcher(&config);

    /* Watch a specific file, not the directory */
    swatcher_target_desc desc = {
        .path = filepath,
        .is_recursive = false,
        .events = SWATCHER_EVENT_ALL,
    ...
    write_file(other_path, "also modified");
    sw_sleep_ms(600);

    /* Should see event for watched file but not unwatched */
    assert(has_event(SWATCHER_EVENT_MODIFIED, "watched.txt"));
    assert(!has_event(SWATCHER_EVENT_MODIFIED, "unwatched.txt"));
...
int main(void)
{
    ...
    printf("=== FSEvents Backend Tests ===\n");

    RUN_TEST(test_fsevents_backend_exists);
    RUN_TEST(test_fsevents_detect_create);
    RUN_TEST(test_fsevents_detect_modify);
    RUN_TEST(test_fsevents_detect_delete);
    RUN_TEST(test_fsevents_detect_move);
    RUN_TEST(test_fsevents_recursive);
    RUN_TEST(test_fsevents_recursive_rmdir);
    RUN_TEST(test_fsevents_pattern_filtering);
    RUN_TEST(test_fsevents_coalesce);
    RUN_TEST(test_fsevents_file_target);
