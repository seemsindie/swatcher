...
#define STRESS_DIR "stress_inotify_tmp"
...
static void stress_rapid_create(void)
{
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        snprintf(filepath, sizeof(filepath), "%s/file_%04d.txt", stress_dir_abs, i);
    ...
    printf("[%d/1000 CREATED] ", created);
    assert(created >= 900); /* Allow some missed events under extreme load */
...
static void stress_rapid_modify(void)
{
    ...
    char filepath[SW_PATH_MAX];
    snprintf(filepath, sizeof(filepath), "%s/rapid_modify.txt", stress_dir_abs);
    ...
    /* 1000 rapid writes to same file */
    for (int i = 0; i < 1000; i++) {
        char buf[64];
...
static void stress_deep_nesting(void)
{
    create_stress_dir();
    ...
    /* Create 50-level nested dirs at runtime */
    char current_dir[SW_PATH_MAX];
    strncpy(current_dir, stress_dir_abs, SW_PATH_MAX - 1);
    current_dir[SW_PATH_MAX - 1] = '\0';

    for (int i = 0; i < 50; i++) {
        char next_dir[SW_PATH_MAX];
        snprintf(next_dir, sizeof(next_dir), "%s/d%d", current_dir, i);
        mkdir(next_dir, 0755);
        sw_sleep_ms(50); /* Give inotify time to pick up and watch new dir */
        strncpy(current_dir, next_dir, SW_PATH_MAX - 1);
    }

    /* Create a file at the bottom */
    char deepfile[SW_PATH_MAX];
    snprintf(deepfile, sizeof(deepfile), "%s/deep.txt", current_dir);
    write_file(deepfile, "deep");
    sw_sleep_ms(500);

    int total = get_event_count();
    printf("[%d events, depth 50] ", total);
    /* Should have events for directory creation plus the file */
    assert(total >= 50);
...
static void stress_churn(void)
{
    ...
    char filepath[SW_PATH_MAX];
    snprintf(filepath, sizeof(filepath), "%s/churn.txt", stress_dir_abs);

    /* Rapidly create+delete the same file */
    for (int i = 0; i < 200; i++) {
    ...
    int total = get_event_count();
    printf("[%d events from 200 create+delete cycles] ", total);
